...
def get_random_word():
    try:
        ...
        return abort(make_response('Fail to get random word from resource', 503))
...
def get_wiki_for_word(word):
    try:
        ...
        return abort(make_response('Cannot find wiki article for given word', 404))
    # all other exceptions
    except external_api_exceptions.ExternalApiError:
        return abort(make_response('Fail to get article from resource', 503))
    # Save to collection
...
def get_joke():
    try:
        ...
        return abort(make_response('Fail to get joke from resource', 503))
